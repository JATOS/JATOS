...
public class IOUtilsTest {
    ...
    @Test
    public void testTmpDirUsesConfiguredTmpPath() {
        String tmpPath = Path.of(System.getProperty("java.io.tmpdir"), "jatos-test").toString();
        commonStatic.when(Common::getTmpPath).thenReturn(tmpPath);

        assertEquals(Path.of(tmpPath), IOUtils.tmpDir());
    }
    ...
    @Test
    public void testMoveFileMovesFileWithoutReplacingExistingTarget() throws Exception {
        Path source = tmp.newFile("moveFileSource.txt").toPath();
        Path target = tmp.getRoot().toPath().resolve("moveFileTarget.txt");
        Files.write(source, "source".getBytes(StandardCharsets.UTF_8));

        IOUtils.moveFile(source, target, false);
    ...
    @Test
    public void testMoveFileReplacesExistingTarget() throws Exception {
        Path source = tmp.newFile("moveFileReplaceSource.txt").toPath();
        Path target = tmp.newFile("moveFileReplaceTarget.txt").toPath();
        Files.write(source, "new".getBytes(StandardCharsets.UTF_8));
        Files.write(target, "old".getBytes(StandardCharsets.UTF_8));

        IOUtils.moveFile(source, target, true);
    ...
    @Test
    public void testMoveFileThrowsIfSourceDoesNotExist() throws Exception {
        Path source = tmp.getRoot().toPath().resolve("missing-source.txt");
        Path target = tmp.getRoot().toPath().resolve("target.txt");
    ...
    @Test
    public void testMoveFileThrowsIfSourceIsDirectory() throws Exception {
        Path source = tmp.newFolder("moveFileSourceDir").toPath();
        Path target = tmp.getRoot().toPath().resolve("target.txt");
        ...
        try {
            ...
            assertTrue(expected.getMessage().contains("Source is not a regular file"));
        }
    }

    @Test
    public void testMoveFileThrowsIfTargetIsDirectory() throws Exception {
        Path source = tmp.newFile("moveFileTargetDirSource.txt").toPath();
        Path target = tmp.newFolder("moveFileExistingTargetDir").toPath();

        try {
            IOUtils.moveFile(source, target, true);
            fail("Expected FileAlreadyExistsException because target is a directory");
    ...
    @Test
    public void testMoveFileThrowsIfTargetExistsAndReplaceExistingIsFalse() throws Exception {
        Path source = tmp.newFile("moveFileNoReplaceSource.txt").toPath();
        Path target = tmp.newFile("moveFileNoReplaceTarget.txt").toPath();
    ...
    @Test
    public void testMoveRecursivelyMovesDirectoryTree() throws Exception {
        Path source = tmp.newFolder("moveRecursiveSource").toPath();
        Path target = tmp.getRoot().toPath().resolve("moveRecursiveTarget");

        Files.createDirectories(source.resolve("nested"));
        Files.write(source.resolve("root.txt"), "root".getBytes(StandardCharsets.UTF_8));
        Files.write(source.resolve("nested/file.txt"), "nested".getBytes(StandardCharsets.UTF_8));

        IOUtils.moveRecursively(source, target);

        assertFalse(Files.exists(source));
        assertTrue(Files.isDirectory(target));
        assertEquals("root", Files.readString(target.resolve("root.txt")));
        assertEquals("nested", Files.readString(target.resolve("nested/file.txt")));
    }

    @Test
    public void testMoveRecursivelyMovesSingleFile() throws Exception {
        Path source = tmp.newFile("moveRecursiveSingleSource.txt").toPath();
        Path target = tmp.getRoot().toPath().resolve("moveRecursiveSingleTarget.txt");
        Files.write(source, "single".getBytes(StandardCharsets.UTF_8));

        IOUtils.moveRecursively(source, target);

        assertFalse(Files.exists(source));
        assertTrue(Files.exists(target));
        assertEquals("single", Files.readString(target));
    }

    @Test
    public void testMoveRecursivelyThrowsIfTargetExists() throws Exception {
        Path source = tmp.newFolder("moveRecursiveExistingSource").toPath();
        Path target = tmp.newFolder("moveRecursiveExistingTarget").toPath();
    ...
    @Test
    public void testDeleteRecursivelyIfExistsDeletesExistingTree() throws Exception {
        Path root = tmp.newFolder("deleteIfExistsRoot").toPath();
        Files.createDirectories(root.resolve("nested"));
        Files.write(root.resolve("nested/file.txt"), "content".getBytes(StandardCharsets.UTF_8));

        IOUtils.deleteRecursivelyIfExists(root);

        assertFalse(Files.exists(root));
    }

    @Test
    public void testDeleteRecursivelyIfExistsDoesNothingForMissingPathOrNull() throws Exception {
        Path missing = tmp.getRoot().toPath().resolve("missing-delete-if-exists");

        IOUtils.deleteRecursivelyIfExists(missing);
        IOUtils.deleteRecursivelyIfExists(null);
    ...
    @Test
    public void testCheckFileInStudyAssetsDirExists() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootCheckFileExists").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        ioUtils.createStudyAssetsDir("studyCheckFile");
        Path file = assetsRoot.resolve("studyCheckFile").resolve("nested/file.txt");
        Files.createDirectories(file.getParent());
        Files.write(file, "content".getBytes(StandardCharsets.UTF_8));

        assertTrue(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", "nested/file.txt"));
        assertFalse(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", "nested/missing.txt"));
        assertFalse(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", null));
        assertFalse(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", ""));
        assertFalse(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", "../outside.txt"));
        assertFalse(ioUtils.checkFileInStudyAssetsDirExists("studyCheckFile", file.toAbsolutePath().toString()));
    }
    ...
    public void testCheckPath() {
        assertTrue(IOUtils.checkPath("file.txt"));
        assertTrue(IOUtils.checkPath("subdir/file.txt"));
        assertTrue(IOUtils.checkPath("sub-dir/file_name.123"));
        assertTrue(IOUtils.checkPath("../notRejectedByRegexButRejectedBySecureResolution"));

        assertFalse(IOUtils.checkPath(null));
        assertFalse(IOUtils.checkPath(""));
        assertFalse(IOUtils.checkPath("subdir\\file.txt"));
        assertFalse(IOUtils.checkPath("file<name.txt"));
        assertFalse(IOUtils.checkPath("file>name.txt"));
        assertFalse(IOUtils.checkPath("file:name.txt"));
        assertFalse(IOUtils.checkPath("file\"name.txt"));
        assertFalse(IOUtils.checkPath("file|name.txt"));
        ...
        assertFalse(IOUtils.checkPath("file*name.txt"));
        assertFalse(IOUtils.checkPath("file\u0000name.txt"));
        assertFalse(IOUtils.checkPath("file\nname.txt"));
        assertFalse(IOUtils.checkPath("file\u007fname.txt"));
    }

    @Test
    public void testRemoveStudyAssetsDirDoesNothingIfDirectoryDoesNotExist() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootRemoveMissing").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        ioUtils.removeStudyAssetsDir("missingStudyAssetsDir");

        assertFalse(Files.exists(assetsRoot.resolve("missingStudyAssetsDir")));
    }

    @Test
    public void testRemoveStudyAssetsDirThrowsIfPathIsFile() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootRemoveFile").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        Files.write(assetsRoot.resolve("notADirectory"), "file".getBytes(StandardCharsets.UTF_8));

        try {
            ioUtils.removeStudyAssetsDir("notADirectory");
            fail("Expected IOException because study assets path is a file");
        } catch (IOException expected) {
            // ok
        }
    }
    ...
    @Test
    public void testMoveStudyAssetsDirMovesDirectoryIntoStudyAssetsRoot() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootMoveStudyAssets").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        Path source = tmp.newFolder("externalStudyAssetsSource").toPath();
        Files.createDirectories(source.resolve("nested"));
        Files.write(source.resolve("nested/file.txt"), "asset".getBytes(StandardCharsets.UTF_8));

        ioUtils.moveStudyAssetsDir(source, "movedStudyAssets");

        Path target = assetsRoot.resolve("movedStudyAssets");
        assertFalse(Files.exists(source));
        assertTrue(Files.isDirectory(target));
        assertEquals("asset", Files.readString(target.resolve("nested/file.txt")));
    }

    @Test
    public void testMoveStudyAssetsDirThrowsIfTargetAlreadyExists() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootMoveStudyAssetsExists").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        Path source = tmp.newFolder("externalStudyAssetsExistingSource").toPath();
        ioUtils.createStudyAssetsDir("existingTarget");

        try {
            ioUtils.moveStudyAssetsDir(source, "existingTarget");
            fail("Expected IOException because target study assets dir already exists");
        } catch (IOException expected) {
            // ok
        }

        assertTrue(Files.exists(source));
        assertTrue(Files.exists(assetsRoot.resolve("existingTarget")));
    }

    @Test
    public void testRenameStudyAssetsDirCreatesNewDirectoryIfOldDirectoryDoesNotExist() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootRenameMissing").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        ioUtils.renameStudyAssetsDir("missingOldDir", "createdNewDir");

        assertFalse(Files.exists(assetsRoot.resolve("missingOldDir")));
        assertTrue(Files.isDirectory(assetsRoot.resolve("createdNewDir")));
    }

    @Test
    public void testRenameStudyAssetsDirThrowsIfNewDirectoryAlreadyExists() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootRenameExisting").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        ioUtils.createStudyAssetsDir("oldDir");
        ioUtils.createStudyAssetsDir("newDir");

        try {
            ioUtils.renameStudyAssetsDir("oldDir", "newDir");
            fail("Expected IOException because new study assets dir already exists");
        } catch (IOException expected) {
            // ok
        }

        assertTrue(Files.exists(assetsRoot.resolve("oldDir")));
        assertTrue(Files.exists(assetsRoot.resolve("newDir")));
    }
    ...
    @Test
    public void testGetStudyAssetsDirSizeReturnsZeroForMissingDirectory() throws Exception {
        Path assetsRoot = tmp.newFolder("assetsRootSizeMissing").toPath();
        commonStatic.when(Common::getStudyAssetsRootPath).thenReturn(assetsRoot.toAbsolutePath().toString());

        assertEquals(0, ioUtils.getStudyAssetsDirSize("missingStudyAssetsDir"));
    }
    ...
    @Test
    public void testGetResultUploadDirSizeReturnsZeroForMissingDirectory() throws Exception {
        Path uploadsRoot = tmp.newFolder("uploadsRootMissingSize").toPath();
        commonStatic.when(Common::getResultUploadsPath).thenReturn(uploadsRoot.toAbsolutePath().toString());

        assertEquals(0, ioUtils.getResultUploadDirSize(999L));
    }

    @Test
    public void testDeleteFileDeletesExistingFile() throws Exception {
        Path file = tmp.newFile("deleteFileExisting.txt").toPath();
        Files.write(file, "delete me".getBytes(StandardCharsets.UTF_8));

        IOUtils.deleteFile(file).run();

        assertFalse(Files.exists(file));
    }

    @Test
    public void testDeleteFileDoesNothingForMissingFile() {
        Path file = tmp.getRoot().toPath().resolve("deleteFileMissing.txt");
