...
public class AuthenticationAction extends Action<Authenticated> {
    ...
    public CompletionStage<Result> call(Http.Context ctx) {
        // For authentication it's actually enough to check that the email is in Play's session. Play's session is safe
        // from tempering. But we retrieve the user from the database and put it into our RequestScope since we need it
        // later anyway. Storing it in the RequestScope now saves us some database requests later.
        ...
        if (Common.getUserSessionValidation() && !authenticationService
                .isValidSessionId(ctx.session(), loggedInUser.getEmail(), ctx.request().remoteAddress())) {
            authenticationService.clearSessionCookie(ctx.session());
            return callForbiddenDueToInvalidSession(loggedInUser.getEmail(), ctx.request().remoteAddress(),
                    ctx.request().path());
        }

        // Check session timeout
        if (authenticationService.isSessionTimeout(ctx.session())) {
            authenticationService.clearSessionCookieAndUserSessionCache(ctx.session(), loggedInUser.getEmail(),
