...
/**
 * Interface to all Publix classes. With these API methods studies and components (running in the browser) can
...
public interface IPublix {
     ...
     */
    Result startStudy(Http.Request request, StudyLink studyLink);
     ...
     *
     * Expects the study's session data in JSON format and sets them in the study result that belongs to the specified
     ...
     */
    Result setStudySessionData(Http.Request request, StudyResult studyResult);
     ...
     * lastSeenDate field.
     */
    Result heartbeat(Http.Request request, StudyResult studyResult);

    /**
     * HTTP type: PUT/POST request
     *
     * Persists the submitted data in the ComponentResult specified by the given study and component ID. PUT: Already
     * submitted data will be overwritten. POST: Keeps the current data and appends the new data to the end.
     */
     ...
     * HTTP type: GET request
     *
     * Downloads a result file that was previously uploaded. One can limit the search to a specific componentId which is
     ...
     */
    Result abortStudy(Http.Request request, StudyResult studyResult, String message);

    /**
     * HTTP type: GET request
     *
     * Finishes the study run (StudyResult state will be FINISHED or FAIL). Optionally, it can be specified whether the
     * study run was successful and a message can be logged.
     */
     ...
     */
    Result log(Http.Request request, StudyResult studyResult, Component component);
