...
/**
  * Handles group action messages, that is a GroupMsg with a JSON object that
  * contains an 'action' field. It was received by an GroupDispatcher and comes
  * from a client via a group channel.
  *
...
class GroupActionHandler @Inject()(jpa: JPAApi,
  ...
  def handleActionMsg(msg: GroupMsg, groupResultId: Long, studyResultId: Long,
    ...
    logger.debug(s".handleActionMsg: groupResultId $groupResultId, studyResultId $studyResultId, jsonNode ${Json.stringify(msg.json)}")
    val actionValue = (msg.json \ GroupActionJsonKey.Action.toString).as[String]
    val action = GroupAction.withName(actionValue)
    action match {
      case GroupAction.Session =>
        handlePatch(msg.json, groupResultId, studyResultId, registry)
      case GroupAction.Fixed =>
        handleActionFix(groupResultId, studyResultId, registry);
      case _ =>
        List(msgBuilder.buildError(groupResultId, s"Unknown action $action",
          TellWhom.SenderOnly))
    }
  }
    ...
    jpa.withTransaction(asJavaSupplier(() => {
      ...
      try {
        ...
        val success = checkVersionAndPersistSessionData(patchedSessionData,
          groupResult, clientsVersion)
        if (success) {
          val msg1 = msgBuilder.buildSessionPatch(groupResult, studyResultId,
            patch, TellWhom.All)
          val msg2 = msgBuilder.buildSimple(groupResult, GroupAction.SessionAck,
            TellWhom.SenderOnly)
          List(msg1, msg2)
        } else {
          List(msgBuilder.buildSimple(groupResult, GroupAction.SessionFail,
            TellWhom.SenderOnly))
        }
      } catch {
        case e: Exception =>
          logger.warn(s".handlePatch: groupResultId $groupResultId, json ${Json.stringify(json)}, ${e.getClass.getName}: ${e.getMessage}")
          List(msgBuilder.buildSimple(groupResult, GroupAction.SessionFail,
            TellWhom.SenderOnly))
      }
    }))
  ...
  private def patchSessionData(patch: JsValue, groupResult: GroupResult) = {
  ...
  private def checkVersionAndPersistSessionData(sessionData: JsValue,
    ...
    if (groupResult != null && sessionData != null && groupResult.getGroupSessionVersion == version) {
